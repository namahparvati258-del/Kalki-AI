...
def speak(text):
    """veda ko bulwane ke liye function"""
...
def execute_task(query):
    ...
    # Sabse pehle identity check karega
    if "tell me about yourself" in query:
        speak("I am the Virtual Engine for Data & Automation AI.")
        return
    
    elif "meaning of your name" in query:
        speak("VEDA stands for Virtual Engine for Data and Automation. My name is derived from the Sanskrit word for Knowledge")
        return
    ...
    # 5. VOLUME & MUSIC
        ...
        song = query.replace("play", "").replace("veda", "").strip()
    ...
    # 6. OPEN APPS
        ...
        app = query.replace("open", "").replace("veda", "").strip()
        if "whatsapp" in app:
    ...
    # 7. AI CHAT
        ...
        try:
            chat_completion = client.chat.completions.create(
                messages=[
                    {"role": "system", "content": "You are veda. ALWAYS reply in English. Max 10 words. Use 'Sir' once."},
...
if __name__ == "__main__":
    is_awake = False
    print("veda Standby. Say 'veda On' to start.")
    ...
    while True:
        ...
        # 2. SLEEP LOGIC
        elif "veda off"  in query:
            is_awake = False
            speak("Going to standby.")
            continue
        elif "veda exit" in query:
            is_awake = False
            speak("your AI is closed,good by sir ")
            sys.exit()

        # 3. COMMAND EXECUTION
        elif is_awake and "veda" in query:
            clean_query = query.replace("veda", "").strip()
            if clean_query != "":
