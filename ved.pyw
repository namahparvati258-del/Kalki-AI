...
def speak(text):
    """ved ko bulwane ke liye function"""
...
def execute_task(query):
    ...
    # 5. VOLUME & MUSIC 
        ...
        song = query.replace("play", "").replace("ved", "").strip()
    ...
    # 6. OPEN APPS
        ...
        app = query.replace("open", "").replace("ved", "").strip()
        if "whatsapp" in app:
    ...
    # 7. AI CHAT
        ...
        try:
            chat_completion = client.chat.completions.create(
                messages=[
                    {"role": "system", "content": "You are ved. ALWAYS reply in English. Max 10 words. Use 'Sir' once."},
...
if __name__ == "__main__":
    is_awake = False
    print("ved Standby. Say 'ved On' to start.")
    ...
    while True:
        ...
        # 2. SLEEP LOGIC
        elif "ved off" in query:
            is_awake = False
            speak("Going to standby.")
            continue
            
        # 3. COMMAND EXECUTION
        elif is_awake and "ved" in query:
            clean_query = query.replace("ved", "").strip()
            if clean_query != "":
