...
def speak(text):
    """kalki ko bulwane ke liye function"""
...
def execute_task(query):
    ...
    # 5. VOLUME & MUSIC
        ...
        song = query.replace("play", "").replace("kalki", "").strip()
    ...
    # 6. OPEN APPS
        ...
        app = query.replace("open", "").replace("kalki", "").strip()
        if "whatsapp" in app:
    ...
    # 7. AI CHAT
        ...
        try:
            chat_completion = client.chat.completions.create(
                messages=[
                    {"role": "system", "content": "You are kalki. ALWAYS reply in English. Max 10 words. Use 'Sir' once."},
...
if __name__ == "__main__":
    is_awake = False
    print("kalki Standby. Say 'kalki On' to start.")
    ...
    while True:
        ...
        # 2. SLEEP LOGIC
        elif "kalki off" in query:
            is_awake = False
            speak("Going to standby.")
            continue
        elif "kalki stop" in query:
            is_awake = False
            speak("your AI is closed,good by sir ")
            sys.exit()

        # 3. COMMAND EXECUTION
        elif is_awake and "kalki" in query:
            clean_query = query.replace("kalki", "").strip()
            if clean_query != "":
